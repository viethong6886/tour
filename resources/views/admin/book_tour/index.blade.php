                                    ...
                                    @foreach($bookTours as $book)
                                        <tr>
                                            ...
                                            <td style="vertical-align: middle; width: 20%" class="title-content">
                                                ...
                                                <p><b>Địa chỉ</b>: {{  isset($book->user) ? $book->user->address : '' }}</p>
                                            </td>
                                            <td style="vertical-align: middle; width: 35%" class="title-content">                                            
                                                ...
                                                <p><b>Số phòng đơn @if($book->fee_singer_room != 0) (có phí  ) @endif :</b> {{ $book->number_singer_room }} - <b>Thành tiền</b>: {{ number_format($book->number_singer_room*500000+$tour->fee_singer_room, 0,',','.') }} vnd</p>
                                                <p><b>Số phòng đôi @if($book->fee_two_room != 0) (có phí  ) @endif :</b> {{ $book->number_two_room }} - <b>Thành tiền</b>: {{ number_format($book->number_two_room*800000+$tour->fee_two_room, 0,',','.') }} vnd</p>
                                                <p><b>Số phòng ba @if($book->fee_three_room != 0) (có phí  ) @endif :</b> {{ $book->number_three_room }} - <b>Thành tiền</b>: {{ number_format($book->number_three_room*1000000+$tour->fee_three_room, 0,',','.') }} vnd</p>
                                                
                                                @php
                                                    $totalPrice = ($book->b_number_adults*$book->b_price_adults) + ($book->b_number_children*$book->b_price_children)+ ($book->b_number_child6*$book->b_price_child6)+ ($book->b_number_child2*$book->b_price_child2)+($book->number_singer_room*500000+$book->fee_singer_room)+($book->number_two_room*500000+$book->fee_two_room)+($book->number_three_room*500000+$book->fee_three_room)
                                                @endphp
                                            ...
                                            <td style="vertical-align: middle; width: 11%">
                                                <button type="button" class="btn btn-block btn-xs">{{ $book->b_status }}</button>
                                            </td>
                                            @if(Auth::user()->can(['full-quyen-quan-ly', 'xoa-va-cap-nhat-trang-thai']))
                                            <td style="vertical-align: middle; width: 17%">
                                                <div class="btn-group">
                                                    ...
                                                    <a class="btn btn-info" target="_blank" href="{{ route('page.prinft_order',$book->id) }}">Xuất PDF</a>
