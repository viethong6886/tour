...
class TourController extends Controller
{
    ...
    public function postBookTour(BookTourRequest $request, $id)
    {
        ...
        try {
            $customerData = array(); // Initialize an empty array

            foreach (request()->all() as $key => $value) {
                if (strpos($key, 'customer') !== false || strpos($key, 'dob') !== false || strpos($key, 'gender') !== false) {
                    // The key contains the strings "customer", "dob", and "gender"
                    // Add the data to the $customerData array
                    $customerNumber = substr($key, -1); // Extract the customer number from the key
                    $customerData[$customerNumber] = array(
                        'name' => request()->input("customer_$customerNumber"),
                        'dob' => request()->input("dob_$customerNumber"),
                        'gender' => request()->input("gender_$customerNumber")
                    );
                }
            }
            ...
            // $params['number_singer_room'] = $params['number_singer_room'];
            $params['fee_singer_room'] = $params['fee_singer_room'] == '0' ? 0 : '500000' ;

            // $params['number_two_room'] = $params['number_two_room'];
            $params['fee_two_room'] = $params['fee_two_room'] == '0' ? 0 : '500000' ;

            // $params['number_three_room'] = $params['number_three_room'];
            $params['fee_three_room'] = $params['fee_three_room'] == '0' ? 0 : '500000' ;

            $params['otherPeople'] = json_encode($customerData) ;
